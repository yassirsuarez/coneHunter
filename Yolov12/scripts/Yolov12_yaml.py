...
def main():
    # Percorsi basati sulla posizione dello script
    script_dir = os.path.dirname(os.path.abspath(__file__))  # cartella dello script
    project_root = os.path.abspath(os.path.join(script_dir, "..", ".."))  # root del progetto (due livelli sopra)
    base_path = os.path.join(project_root, "data_split")  # dataset
    data_config_path = os.path.join(base_path, "config.yaml")  # config dataset
    model_weights_path = os.path.join(script_dir, "yolo12s.pt")  # pesi modello
    ...
    if not os.path.exists(model_weights_path):
        print(f"Errore: File dei pesi modello {model_weights_path} non trovato!")
        return
    if not os.path.exists(data_config_path):
        print(f"Errore: File di configurazione dataset {data_config_path} non trovato!")
        return
